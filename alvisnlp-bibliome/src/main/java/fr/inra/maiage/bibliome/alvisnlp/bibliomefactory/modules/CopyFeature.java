...
import fr.inra.maiage.bibliome.alvisnlp.bibliomefactory.modules.CopyFeature.CopyFeatureResolvedObjects;
...
public class CopyFeature extends CorpusModule<CopyFeatureResolvedObjects> {
	...
	public void process(ProcessingContext<Corpus> ctx, Corpus corpus) throws ModuleException {
		CopyFeatureResolvedObjects res = new CopyFeatureResolvedObjects(ctx);
		...
		for (Element t : Iterators.loop(res.target.evaluateElements(evalCtx, corpus))) {
			String value = t.getLastFeature(sourceFeatureName);
			actionCtx.registerSetFeature(t, targetFeatureName, value);
			n++;
		}
	...
	@Override
	protected CopyFeatureResolvedObjects createResolvedObjects(ProcessingContext<Corpus> ctx) throws ResolverException {
		return new CopyFeatureResolvedObjects(ctx);
	}

	class CopyFeatureResolvedObjects extends ResolvedObjects {
		@SuppressWarnings("hiding")
		private final Evaluator target;
		
		private CopyFeatureResolvedObjects(ProcessingContext<Corpus> ctx) throws ResolverException {
			super(ctx, CopyFeature.this);
			target = rootResolver.resolveNullable(CopyFeature.this.target);
		}
	}
