...
public final class <xsl:value-of select="$class-name"/> extends <xsl:value-of select="@full-name"/> {
    ...
    @Override
    @fr.inra.maiage.bibliome.alvisnlp.core.module.lib.Param(publicName="<xsl:value-of select="@public"/>", nameType="<xsl:value-of select="@name-type"/>", mandatory=<xsl:value-of select="@mandatory"/>)
    ...
    <xsl:for-each select="funlib">
      ...
      @Override
      <xsl:value-of select="concat(@access, ' ', @type, ' ', @factory, '(fr.inra.maiage.bibliome.alvisnlp.core.node.evaluator.FunctionLibrary parent) { return new ', $internal-class, '(parent); }')"/>

      private static final class <xsl:value-of select="concat($internal-class, ' extends ', @type)"/> {
        private <xsl:value-of select="$internal-class"/>(fr.inra.maiage.bibliome.alvisnlp.core.node.evaluator.FunctionLibrary parent) {
	  super();
	  setParent(parent);
	...
	<xsl:variable name="args">
	  <xsl:for-each select="arg">
	    ...
	    fr.inra.maiage.bibliome.alvisnlp.core.node.evaluator.Evaluator <xsl:value-of select="@name"/>
	  </xsl:for-each>
	</xsl:variable>

	private final class <xsl:value-of select="$evaluator-class"/> extends <xsl:value-of select="@evaluator-super"/> {
	  <xsl:for-each select="arg">
	  private final fr.inra.maiage.bibliome.alvisnlp.core.node.evaluator.Evaluator <xsl:value-of select="@name"/>;
	  </xsl:for-each>
	  ...
	  @Override
	  public <xsl:value-of select="concat(@return-type, ' ', @evaluator-method)"/>(fr.inra.maiage.bibliome.alvisnlp.core.node.Node node) {
	...
	private final fr.inra.maiage.bibliome.alvisnlp.core.node.evaluator.Function <xsl:value-of select="@javaName"/> = new fr.inra.maiage.bibliome.alvisnlp.core.node.evaluator.AbstractFunction(&quot;<xsl:value-of select="@publicName"/>&quot;, fr.inra.maiage.bibliome.alvisnlp.core.node.expression.Type.<xsl:value-of select="@type"/>, <xsl:value-of select="@minArgs"/>, <xsl:value-of select="@maxArgs"/>, <xsl:value-of select="@deterministic"/>) {
	  @Override
	  protected fr.inra.maiage.bibliome.alvisnlp.core.node.evaluator.Evaluator doGetEvaluator(fr.inra.maiage.bibliome.alvisnlp.core.node.evaluator.Evaluator[] args) {
    ...
    <xsl:for-each select="timed-method">
      ...
      {
        fr.inra.maiage.bibliome.util.Timer&lt;fr.inra.maiage.bibliome.alvisnlp.core.module.TimerCategory> timer = getTimer(_1,
        "<xsl:value-of select="@task"/>",
        <xsl:value-of select="concat('fr.inra.maiage.bibliome.alvisnlp.core.module.TimerCategory.', @category)"/>,
        true
        );
