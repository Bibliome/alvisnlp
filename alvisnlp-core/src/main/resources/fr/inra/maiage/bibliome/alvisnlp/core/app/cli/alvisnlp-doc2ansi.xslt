...
<xsl:stylesheet version="1.0"
	 ...
	 xmlns:ansi="xalan://fr.inra.maiage.bibliome.alvisnlp.core.app.cli.XSLTAnsiEscape"
	 >
...
<xsl:text>
  ...
  <xsl:template match="module-doc|plan-doc">
    ...
    <xsl:value-of select="ansi:bright(ansi:underline($parameters))"/>
    <xsl:text>
</xsl:text>
  ...
  <xsl:template match="synopsis|description|string-conversion|xml-conversion">
    <xsl:variable name="title">
      <xsl:choose>
	<xsl:when test="name() = 'synopsis'"><xsl:value-of select="ansi:bright(ansi:underline($synopsis))"/></xsl:when>
	<xsl:when test="name() = 'description'"><xsl:value-of select="ansi:bright(ansi:underline($description))"/></xsl:when>
	<xsl:when test="name() = 'string-conversion'"><xsl:value-of select="ansi:bright(ansi:underline($string-conversion))"/></xsl:when>
	<xsl:when test="name() = 'xml-conversion'"><xsl:value-of select="ansi:bright(ansi:underline($xml-conversion))"/></xsl:when>
      </xsl:choose>
    </xsl:variable>
    <xsl:text>
  ...
  <xsl:template match="em"><xsl:value-of select="ansi:italic(string(.))"/></xsl:template>
