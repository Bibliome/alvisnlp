...
public final class <xsl:value-of select="@simple-name"/> extends <xsl:value-of select="@base-class"/> {
    ...
    @Override
    public fr.inra.maiage.bibliome.alvisnlp.core.documentation.Documentation getDocumentation() {
        return new fr.inra.maiage.bibliome.alvisnlp.core.documentation.ResourceDocumentation("<xsl:value-of select="@resource-bundle-name"/>");
    }

    @Override
    public fr.inra.maiage.bibliome.alvisnlp.core.corpus.expressions.Evaluator resolveExpression(fr.inra.maiage.bibliome.alvisnlp.core.corpus.expressions.LibraryResolver resolver, java.util.List&lt;String> ftors, java.util.List&lt;fr.inra.maiage.bibliome.alvisnlp.core.corpus.expressions.Expression> args) throws fr.inra.maiage.bibliome.alvisnlp.core.corpus.expressions.ResolverException {
        if (!ftors.isEmpty()) {
	...
	private final String <xsl:value-of select="concat('ftor', position())"/>;
      ...
      <xsl:for-each select="arg">
	private final fr.inra.maiage.bibliome.alvisnlp.core.corpus.expressions.Evaluator <xsl:value-of select="concat('arg', position())"/>;
      </xsl:for-each>
	...
	<xsl:if test="position() != 1">, </xsl:if>String <xsl:value-of select="concat('ftor', position())"/>
      ...
      <xsl:for-each select="arg">
	<xsl:if test="count(../ftor) != 0 or position() != 1">, </xsl:if>fr.inra.maiage.bibliome.alvisnlp.core.corpus.expressions.Evaluator <xsl:value-of select="concat('arg', position())"/>
      </xsl:for-each>
      ) {
      <xsl:for-each select="ftor">
	...
	this.<xsl:value-of select="concat('arg', position())"/> = <xsl:value-of select="concat('arg', position())"/>;
      ...
      @Override
      public <xsl:value-of select="concat(@return-type, ' ', @evaluation-method)"/>(fr.inra.maiage.bibliome.alvisnlp.core.corpus.expressions.EvaluationContext ctx, fr.inra.maiage.bibliome.alvisnlp.core.corpus.Element elt) {
	...
	@Override
	public void evaluateString(fr.inra.maiage.bibliome.alvisnlp.core.corpus.expressions.EvaluationContext ctx, fr.inra.maiage.bibliome.alvisnlp.core.corpus.Element elt, fr.inra.maiage.bibliome.util.StringCat strcat) {
	  strcat.append(evaluateString(ctx, elt));
	}
      </xsl:if>

	@Override
	public void collectUsedNames(fr.inra.maiage.bibliome.alvisnlp.core.module.NameUsage nameUsage, String defaultType) throws fr.inra.maiage.bibliome.alvisnlp.core.module.ModuleException {
	<xsl:for-each select="ftor">
